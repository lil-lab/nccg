 ...
 *******************************************************************************/
package edu.cornell.cs.nlp.spf.parser.ccg.shiftreduce.sloppy;
 ...
 * @author Yoav Artzi
 */
public class SimpleWordSkippingLexicalGenerator<DI extends Sentence, MR>
		implements ISentenceLexiconGenerator<DI, MR> {
	public static final String			SKIPPING_LEXICAL_ORIGIN	= "skipping";

	private static final long			serialVersionUID		= 7784764619676832428L;
	...
	public Set<LexicalEntry<MR>> generateLexicon(DI sample) {
		...
		for (int j = 0; j < tokens.size(); j++) {
			...
			lexicalEntries.add(new LexicalEntry<MR>(tokens.sub(j, j + 1),
