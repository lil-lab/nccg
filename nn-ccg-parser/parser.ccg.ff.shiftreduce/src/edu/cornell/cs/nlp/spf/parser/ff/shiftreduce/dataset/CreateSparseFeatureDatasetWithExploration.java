...
public class CreateSparseFeatureDatasetWithExploration<SAMPLE extends IDataItem<?>, DI extends ILabeledDataItem<SAMPLE, ?>, MR> 
					extends AbstractCreateDatasetWithExploration<SparseFeatureDataset<MR>, SAMPLE, DI, MR> {
	
	public static final ILogger	LOG = LoggerFactory.create(CreateCompositeDecisionDataset.class);
	private final Random rnd;
	private final double p;
	
	public CreateSparseFeatureDatasetWithExploration(IDataCollection<DI> trainingData, 
	...
	@Override
	protected List<SparseFeatureDataset<MR>> preProcessDataPointsWithExploration(
		...
		List<SparseFeatureDataset<MR>> dataset = new ArrayList<SparseFeatureDataset<MR>>();
		...
		while(options.size() != 1 || currentStep != parseTreesLen[options.iterator().next()] - 1) { //TODO check this
			...
			SparseFeatureDataset<MR> pt = new SparseFeatureDataset<MR>(possibleActionFeatures, optionIx, tkString, possibleActions);
			...
			for(int option: options) {
				...
				LOG.debug("Option Up %s, index %s, option %s %s %s", optionUp, option, optionDState,
												optionDState.hashCode(), optionDState.getDebugHashCode());
		...
		if(words != 0) {
			throw new RuntimeException("All words not consumed. This looks like a bug!");
		}
	...
	public static class Creator<SAMPLE extends IDataItem<?>, DI extends ILabeledDataItem<SAMPLE, ?>, MR> 
				implements IResourceObjectCreator<CreateSparseFeatureDatasetWithExploration<SAMPLE, DI, MR>> {

		private final String type;

		public Creator() {
			this("data.neural.sparsefeature.exploration");
		}
		...
		@Override
		public CreateSparseFeatureDatasetWithExploration<SAMPLE, DI, MR> create(Parameters params, IResourceRepository repo) {
			...
			return new CreateSparseFeatureDatasetWithExploration<SAMPLE, DI, MR>(trainingData, parser, validator, beamSize, 
									parsingFilterFactory, compositeLexicon, tempLexicon, graphAmrParser, amrSupervisedFilterFactory);
		}
