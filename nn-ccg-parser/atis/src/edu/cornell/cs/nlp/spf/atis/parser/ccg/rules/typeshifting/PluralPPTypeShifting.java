 ...
 */
public class PluralPPTypeShifting implements IUnaryParseRule<LogicalExpression> {

	private static final long		serialVersionUID	= -5817151499941237319L;
	...
	public PluralPPTypeShifting(
		...
		this(categoryServices, "shift_pp_plural");
	}
		...
		if (category.getSyntax().equals(Syntax.PP)) {
			...
			if (raisedSemantics != null) {
				return new ParseRuleResult<LogicalExpression>(name,
						Category.create(Syntax.PP, raisedSemantics));
			}
		}
		return null;
	}
	...
	public boolean equals(Object obj) {
		...
		final PluralPPTypeShifting other = (PluralPPTypeShifting) obj;
		if (aPred == null) {
			if (other.aPred != null) {
				return false;
			}
	...
	public boolean isValidArgument(Category<LogicalExpression> category,
			SentenceSpan span) {
		return Syntax.PP.equals(category.getSyntax());
	}

	private LogicalExpression typeShiftSemantics(LogicalExpression sem) {
