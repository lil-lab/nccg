...
import edu.uw.cs.lil.tiny.parser.filter.IParsingFilterFactory;
import edu.uw.cs.lil.tiny.parser.filter.StubFilterFactory;
...
public class LossSensitivePerceptronCKY<MR, DI extends ILossDataItem<Sentence, MR>> {
	...
	private final IParsingFilterFactory<DI, MR>								filterFactory;
	...
	public LossSensitivePerceptronCKY(int numIterations, double margin,
			...
			IParsingFilterFactory<DI, MR> filterFactory) {
			...
			for (final DI dataItem : trainingData) {
				...
				final IParserOutput<MR> generateLexiconParserOutput = parser
						.parse(dataItem.getSample(),
								filterFactory.create(dataItem), dataItemModel,
								false, generatedLexicon,
								lexiconGenerationBeamSize,
								lexiconGenerationCellFactory);

				stats.recordGenerationParsing(generateLexiconParserOutput
						.getParsingTime());
				final List<? extends IDerivation<MR>> allGenerationParses = generateLexiconParserOutput
						.getAllDerivations();
				...
				final List<IDerivation<MR>> validBestGenerationParses = new LinkedList<IDerivation<MR>>();
				double currentMinLoss = Double.MAX_VALUE;
				double currentMaxModelScore = -Double.MAX_VALUE;
				LOG.info("Generation parses:");
				for (final IDerivation<MR> parse : allGenerationParses) {
				...
				for (final IDerivation<MR> parse : validBestGenerationParses) {
				...
				for (final IDerivation<MR> parse : allGenerationParses) {
					if (!validBestGenerationParses.contains(parse)
						...
						for (final IDerivation<MR> bestParse : validBestGenerationParses) {
				...
				final Collection<? extends IDerivation<MR>> modelParses = modelParserOutput
						.getAllDerivations();
				...
				// (loss, parse)
				final Pair<List<Pair<Double, IDerivation<MR>>>, List<Pair<Double, IDerivation<MR>>>> goodBadSetsPair = createOptimalNonOptimalSets(
						dataItem, modelParses);
				final List<Pair<Double, IDerivation<MR>>> optimalParses = goodBadSetsPair
						.first();
				final List<Pair<Double, IDerivation<MR>>> nonOptimalParses = goodBadSetsPair
						.second();
				...
				for (final Pair<Double, IDerivation<MR>> pair : optimalParses) {
					logParse(dataItem, pair.second(), pair.first(), false,
							dataItemModel);
				}

				LOG.info("Non-optimal parses:");
				for (final Pair<Double, IDerivation<MR>> pair : nonOptimalParses) {
					logParse(dataItem, pair.second(), pair.first(), false,
							dataItemModel);
				}
				...
				final List<Pair<Double, IDerivation<MR>>> violatingOptimalParses = new LinkedList<Pair<Double, IDerivation<MR>>>();
				final List<Pair<Double, IDerivation<MR>>> violatingNonOptimalParses = new LinkedList<Pair<Double, IDerivation<MR>>>();
				...
				for (final Pair<Double, IDerivation<MR>> optimalParse : optimalParses) {
					int nonOptimalParsesCounter = 0;
					for (final Pair<Double, IDerivation<MR>> nonOptimalParse : nonOptimalParses) {
				...
				for (final Pair<Double, IDerivation<MR>> pair : violatingOptimalParses) {
					logParse(dataItem, pair.second(), pair.first(), true,
							dataItemModel);
				}

				LOG.info("Violating non-optimal parses: ");
				for (final Pair<Double, IDerivation<MR>> pair : violatingNonOptimalParses) {
					logParse(dataItem, pair.second(), pair.first(), false,
							dataItemModel);
				}
	 ...
	 */
	private Pair<List<Pair<Double, IDerivation<MR>>>, List<Pair<Double, IDerivation<MR>>>> createOptimalNonOptimalSets(
			ILossDataItem<Sentence, MR> dataItem,
			Collection<? extends IDerivation<MR>> parseResults) {
		double minLoss = Double.MAX_VALUE;
		final List<Pair<Double, IDerivation<MR>>> optimalParses = new LinkedList<Pair<Double, IDerivation<MR>>>();
		final List<Pair<Double, IDerivation<MR>>> nonOptimalParses = new LinkedList<Pair<Double, IDerivation<MR>>>();
		for (final IDerivation<MR> parseResult : parseResults) {
	...
	private void logParse(IDataItem<Sentence> dataItem, IDerivation<MR> parse,
	...
	public static class Builder<MR, DI extends ILossDataItem<Sentence, MR>> {
		...
		private IParsingFilterFactory<DI, MR>								filterFactory								= new StubFilterFactory<DI, MR>();
		...
		public LossSensitivePerceptronCKY<MR, DI> build() {
			return new LossSensitivePerceptronCKY<MR, DI>(
					...
					genlex, filterFactory);
		}

		public Builder<MR, DI> setFilterFactory(
				IParsingFilterFactory<DI, MR> filterFactory) {
			this.filterFactory = filterFactory;
			return this;
		}
