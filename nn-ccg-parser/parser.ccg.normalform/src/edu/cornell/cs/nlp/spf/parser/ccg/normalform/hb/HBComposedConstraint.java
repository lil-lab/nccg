...
public class HBComposedConstraint implements INormalFormConstraint {
	...
	public static final ILogger	LOG					= LoggerFactory.create(HBComposedConstraint.class);
	...
	public boolean isValid(IArrayRuleNameSet leftGeneratingRules,
		...
		if (constraint1IsApplicable || constraint2IsApplicable
				|| includeTypeRaising) {
			final int len = secondaryRules/*primaryRules*/.numRuleNames();
			for (int i = 0; i < len; ++i) {
				final RuleName rule = secondaryRules/*primaryRules*/.getRuleName(i);
				...
				if (constraint1IsApplicable && isComposition
						&& directionEqual) {
					/*LOG.debug("False 1: Constraint application: "+consideredIsApplication+" and composition:"+
							consideredIsComposition+" and considered order:"+consideredOrder+" and isComposition:"+isComposition
							+" and direction equal "+directionEqual);*/
					return false;
				}
				/*LOG.debug("Passed 1: Constraint application: "+consideredIsApplication+" and composition:"+
						consideredIsComposition+" and considered order:"+consideredOrder+" and isComposition:"+isComposition
						+" and direction equal "+directionEqual);*/
