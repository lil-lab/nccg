...
public abstract class LogicalExpression implements
	...
	public boolean equals(LogicalExpression exp,
		...
		if(exp != null && LOG.getLogLevel() == LogLevel.DEBUG) {
			boolean finalResult = doEquals(exp, mapping);
			String s1 = this.hashCodeCalculated + " and " + exp.hashCodeCalculated;
			String s2 = "Hashcode " + (exp.hashCode() == hashCode()) + " for " + this + " and " + exp + " eventualy " + finalResult;
			if(exp.hashCode() != hashCode() && finalResult) {
				LOG.debug("%s \n %s", s1, s2);
				LOG.debug("Strange behaviour. Found hashCode %s and %s", this.hashCode(), exp.hashCode());
				
				if(exp instanceof Lambda && this instanceof Lambda) {
					Lambda l1 = (Lambda)(this);
					Lambda l2 = (Lambda)exp;
					LOG.debug("This is a lambda. Body Hashcodes %s %s", l1.getBody().hashCode(), l2.getBody().hashCode());
					LOG.debug("Argument Hashcodes %s %s %s %s", l1.getArgument().hashCode(), l2.getArgument().hashCode(), l1.getArgument(), l2.getArgument());
					LOG.debug("Type Hashcodes %s %s %s %s", l1.getType().hashCode(), l2.getType().hashCode(), l1.getType(), l2.getType());
					LOG.debug("l1 and l2 hashcodes %s %s", l1.hashCode(), l2.hashCode());
					LOG.debug("l1 cal %s and l2 cal %s", l1.calcHashCode(), l2.calcHashCode());
				}
			}
		}
