...
public class Literal extends LogicalExpression {
	...
	protected boolean doEquals(LogicalExpression exp,
		...
		if (getClass() != exp.getClass()) { LOG.debug("Literal Fail code 0");
			return false;
		}
		final Literal other = (Literal) exp;
		if (!predicate.equals(other.predicate, mapping)) { LOG.debug("Literal Fail code 1");
			return false;
		}
		if (arguments.length != other.arguments.length) {  LOG.debug("Literal Fail code 2");
			return false;
		}
		...
		if (getPredicateType().isOrderSensitive()) {
			for (int i = 0; i < arguments.length; ++i) {
				if (!arguments[i].equals(other.arguments[i], mapping)) {
					LOG.debug("Literal Fail code 3 %s %s and %s", arguments[i], other.arguments[i], mapping);
					return false;
				}
			}
		} else {
