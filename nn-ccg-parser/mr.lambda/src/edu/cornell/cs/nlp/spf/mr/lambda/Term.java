...
public abstract class Term extends LogicalExpression {
	...
	protected boolean doEquals(LogicalExpression exp) {
		...
		if (exp == null) { LOG.debug("Term Failed code 1 %s", this);
			return false;
		}
		if (getClass() != exp.getClass()) { LOG.debug("Term Failed code 2 %s", this);
			return false;
		}
		final Term other = (Term) exp;
		if (type == null) {
			if (other.type != null) { LOG.debug("Term Failed code 3 %s", this);
				return false;
			}
		} else if (!type.equals(other.type)) { LOG.debug("Term Failed code 4 %s", this);
			return false;
		}
		return true;
	}
