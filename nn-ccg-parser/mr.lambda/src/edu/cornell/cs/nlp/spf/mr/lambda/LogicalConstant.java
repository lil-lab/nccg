...
public class LogicalConstant extends Term {
	...
	protected boolean doEquals(LogicalConstant exp) {
		...
		if (!super.doEquals(exp)) { LOG.debug("Exp Failed code 1 %s", this);
			return false;
		}
		if (name == null) {
			if (exp.name != null) { LOG.debug("Exp Failed code 2 %s", this);
				return false;
			}
		} else if (!name.equals(exp.name)) { LOG.debug("Exp Failed code 3 %s", this);
			return false;
		}
		return true;
	}
