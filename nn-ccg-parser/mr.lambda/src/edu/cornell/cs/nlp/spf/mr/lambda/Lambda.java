...
public class Lambda extends LogicalExpression {
	...
	protected boolean doEquals(LogicalExpression exp,
		...
		if (getClass() != exp.getClass()) {	LOG.debug("Lambda Fail code 1");
			return false;
		}
		final Lambda other = (Lambda) exp;
		if (!type.equals(other.type)) { 	LOG.debug("Lambda Fail code 2");
			return false;
		}
		...
		} else { 	LOG.debug("Lambda Fail code 3");
			return false; 
		}
		...
		if(!ret) LOG.debug("Lambda Fail code 4");
