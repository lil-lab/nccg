...
public class ShiftingRuleFeatureSet<DI extends IDataItem<?>, MR> implements
	...
	public void setFeatures(IParseStep<MR> parseStep, IHashVector feats,
		...
		if(parseStep instanceof AbstractShiftReduceStep) {
			AbstractShiftReduceStep<MR> shiftReduceParseStep = 
											((AbstractShiftReduceStep<MR>)parseStep);
			if(shiftReduceParseStep.isUnary()) {
				final UnaryRuleName unaryRuleName = (UnaryRuleName)shiftReduceParseStep.getRuleName();
				feats.add(tag, unaryRuleName.toString(), 1.0 * scale);
			}
		} else {
			//Below code is used by CKY  or parsers that handle unary rule via overloading
			//ShiftReduce does not use OverloadedRuleName hence condition below won't be setup.
