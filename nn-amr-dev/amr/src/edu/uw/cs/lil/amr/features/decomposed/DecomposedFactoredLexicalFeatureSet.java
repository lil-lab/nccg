 ...
 * optionally create features for the template and the lexeme. This feature set
 * is decomposed, so no paired features are created. The feature is tagged using
 * the IDs of the template and lexeme. IDs are tracked by the feature set. If
 * the a non-factored lexical
 ...
 * @author Yoav Artzi
 */
public class DecomposedFactoredLexicalFeatureSet<DI extends IDataItem<?>>
		extends AbstractLexicalFeatureSet<DI, LogicalExpression> {

	public static final ILogger												LOG					= LoggerFactory
			.create(DecomposedFactoredLexicalFeatureSet.class);
	...
	protected DecomposedFactoredLexicalFeatureSet(
	...
	public static class Builder<DI extends IDataItem<?>> {
		...
		public DecomposedFactoredLexicalFeatureSet<DI> build() {
			return new DecomposedFactoredLexicalFeatureSet<>(ignoreFilter,
	...
	public static class Creator<DI extends IDataItem<?>> implements
			IResourceObjectCreator<DecomposedFactoredLexicalFeatureSet<DI>> {
		...
		@Override
		public DecomposedFactoredLexicalFeatureSet<DI> create(Parameters params,
		...
		public ResourceUsage usage() {
			return ResourceUsage
					.builder(type, DecomposedFactoredLexicalFeatureSet.class)
					.setDescription(
					...
					.addParam("entryScorer", ISerializableScorer.class,
							"Initial scorer for binary lexical entry binary features (default: f(e) = 0.0)")
					.addParam("entryScale", Double.class,
							"Scaling factor for lexical entry binary features (default: 1.0)")
