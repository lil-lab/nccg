...
public class SemanticShiftingFeatureSet<DI extends IDataItem<?>> implements
	...
	public void setFeatures(IParseStep<LogicalExpression> step,
		...
		if (step instanceof AbstractShiftReduceStep) {
			AbstractShiftReduceStep<LogicalExpression> shiftReduceParseStep = 
								((AbstractShiftReduceStep<LogicalExpression>)step);
			if(shiftReduceParseStep.isUnary()) {
				final UnaryRuleName rule = (UnaryRuleName)shiftReduceParseStep.getRuleName();
				final LogicalExpression semantics = shiftReduceParseStep.getRoot().getSemantics();
				if (semantics != null) {
		...
		} else { //Below code is used by CKY  or parsers that handle unary rule via overloading
